...
while not game_over:
    ...
    while victory != True:
        ...
        if coin != 0:
            coin_input = random.choice(["y", "n"])
            print('Pull a lever (y/n):', coin_input)
            if coin_input == 'y':
                coins += 1
                print('You received 1 coin, your total is now {}.'.format(coins))
        print_options(options)
        user_input = random.choice(["n", "e", "s", "w"])
        print("Direction:", user_input)
        user_input = user_input.upper()
        moves += 1

        while (not (user_input == options[0] or user_input == options[1] or user_input == options[2] or user_input ==
                    options[3])) or user_input == 'X':
